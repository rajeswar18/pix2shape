...
from diffrend.torch.GAN.generator_networks.DCGAN_G import DCGAN_G
...
def create_networks(opt, verbose=True, **params):
    ...
    nb_same_conv = int(opt.gen_nsame_conv)
    ...
    if opt.gen_type == 'mlp':
        ...
        netG = DCGAN_G(splats_img_size, nz, splats_n_dims, ngf, ngpu,
                       ...
                       norm=gen_norm, nb_same_conv=nb_same_conv,
                       upsampling_mode=opt.gen_upsampling_mode)
    ...
    if opt.disc_type == 'cnn':
        if render_img_size == 128:
            netD2 = _netD2(ngpu, 3, ndf, render_img_size,nz,
                          use_sigmoid=use_sigmoid)
            netD3 = _netD3(ngpu, 3, ndf, render_img_size,nz,
                          use_sigmoid=use_sigmoid)
        else:
    ...
    return netG, netG2, netD, netD2, netD3
...
class _netD(nn.Module):
    def __init__(self, ngpu, nc, ndf, isize, nz, use_sigmoid=0):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(7+3, ndf, 4, 2, 1, bias=True),
            #nn.Dropout(p=0.3),
            nn.LeakyReLU(),
            ...
            nn.Conv2d(ndf * 8, ndf * 12, 4, 2, 1, bias=True),

            nn.LeakyReLU(),
            nn.Conv2d(ndf * 12, ndf * 14, 4, 1, 0, bias=True),
            nn.LeakyReLU()
            )
            # state size. (ndf*8) x 4 x 4
        self.main2 = nn.Sequential(nn.Conv2d(ndf * 14, 1, 1, 1, 0, bias=True)
        )
...
class _netD2(nn.Module):
    def __init__(self, ngpu, nc, ndf, isize, nz, use_sigmoid=0):
        ...
        self.main = nn.Sequential(
            # input is (nc) x 64 x 64
            nn.Conv2d(1+3, ndf, 4, 2, 1, bias=True),
            #nn.Dropout(p=0.3),
            nn.LeakyReLU(),
            # state size. (ndf) x 32 x 32
            nn.Conv2d(ndf, ndf * 2, 4, 2, 1, bias=True),
            ...
            # state size. (ndf*8) x 4 x 4
        self.main2 = nn.Sequential(nn.Conv2d(ndf * 8, 1, 1, 1, 0, bias=True)
...
class _netD3(nn.Module):
    def __init__(self, ngpu, nc, ndf, isize, nz, use_sigmoid=0):
        ...
        self.main = nn.Sequential(
            ...
            # state size. (ndf*8) x 4 x 4
        self.main2 = nn.Sequential(nn.Conv2d(ndf * 10, 1, 1, 1, 0, bias=True)
