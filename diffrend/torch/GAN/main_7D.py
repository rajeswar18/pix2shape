...
from diffrend.torch.GAN.twin_networks_7D import create_networks
...
def copy_scripts_to_folder(expr_dir):
    """Copy scripts."""
    shutil.copy("twin_networks_7D.py", expr_dir)
...
class GAN(object):
    ...
    def create_networks(self, ):
        """Create networks."""
        self.netG, _, self.netD, self.netD2, self.netD3 = create_networks(
    ...
    def create_optimizers(self, ):
        ...
        if self.opt.optimizer == 'adam':
            ...
            self.optimizerD2 = optim.Adam(self.netD2.parameters(),
                                         lr=self.opt.lr,
                                         betas=(self.opt.beta1, 0.999))
            self.optimizerD3 = optim.Adam(self.netD3.parameters(),
                                         lr=self.opt.lr,
                                         betas=(self.opt.beta1, 0.999))
            ...
            self.optimizerD2 = optim.RMSprop(self.netD2.parameters(),
                                            lr=self.opt.lr)
            self.optimizerD3 = optim.RMSprop(self.netD3.parameters(),
                                            lr=self.opt.lr)
    ...
    def get_real_samples(self):
        ...
        for idx in range(self.opt.batchSize):
            ...
            if self.opt.render_img_nc == 1:
                ...
                #print(target_normalmap_img_.shape)
                im_n = tch_var_f(
        ...
        with open(file_name, 'wt') as l2_file:
            ...
            for iteration in range(self.opt.n_iter):
                ...
                for j in range(self.opt.critic_iters):
                    ...
                    real_output = self.netD(torch.cat([self.inputv, self.inputv_depth,self.inputv_normal], 1), self.inputv_cond.detach())
                    ...
                    fake_depth = fake_z[:,3:4]
                    fake_normal = fake_z[:,4:]
                    # Do not bp through gen

                    outD_fake = self.netD(fake_z.detach(), self.inputv_cond.detach())
                    ...
                    if self.opt.gp != 'None':
                        gradient_penalty = calc_gradient_penalty(
                            self.netD,  torch.cat([self.inputv.data, self.inputv_depth.data,self.inputv_normal.data], 1), fake_z.data,
                ...
                outG_fake = self.netD(fake_z, self.inputv_cond)
                ...
                if iteration % self.opt.print_interval == 0:
                    ...
                    l2_loss = mse_criterion(fake_depth, self.inputv_depth)
                    ...
                    print('\n[%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_D_real: %.4f  '
                          ...
                          ' z_lr: %.8f,  Disc_grad_norm: %.8f, Gen_grad_norm: %.8f' % (
                ...
                if iteration % (self.opt.save_image_interval) == 0 and iteration % (2*self.opt.save_image_interval) !=0:
                    ...
                    torchvision.utils.save_image(
                        fake_normal.data,
                        os.path.join(self.opt.vis_images,
                                     'output_normal%d.png' % (iteration)),
                        nrow=2, normalize=True, scale_each=True)
                    torchvision.utils.save_image(
                        fake_depth.data,
                        os.path.join(self.opt.vis_images,
                                     'output_depth%d.png' % (iteration)),
                        nrow=2, normalize=True, scale_each=True)
                    imsave((inpath + str(self.iteration_no) +
                             'depthmap_{:05d}.png'.format(iteration)),
                            get_data(fake_depth[0].squeeze()))
                    imsave((inpath + str(self.iteration_no) +
                             'depthmap1_{:05d}.png'.format(iteration)),
                            get_data(fake_depth[1].squeeze()))
                    imsave((inpath + str(self.iteration_no) +
    ...
    def save_networks(self, epoch):
        ...
        # torch.save(self.netD3.state_dict(),
        #            '%s/netD3_epoch_%d.pth' % (self.opt.out_dir, epoch))
